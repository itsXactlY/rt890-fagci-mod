...
static void AT32_EnableIRQ(NVIC_Config_t *pConfig) {
  if (pConfig->bEnable) {
    nvic_irq_enable(pConfig->Irq, pConfig->PreemptPriority,
                    pConfig->SubPriority);
  } else {
...
static void InitGPIO(void) {
  ...
  init.gpio_pins = BOARD_GPIOA_LCD_SCL | BOARD_GPIOA_SPEAKER |
                   BOARD_GPIOA_LCD_SDA | BOARD_GPIOA_LCD_RESX |
                   BOARD_GPIOA_KEY_COL3;
  ...
  init.gpio_pins = 0
                   // SPI Flash
                   | BOARD_GPIOB_SF_CS | BOARD_GPIOB_SF_MISO |
                   BOARD_GPIOB_SF_CLK
                   // BK4819
                   | BOARD_GPIOB_BK4819_SDA | BOARD_GPIOB_BK4819_CS |
                   BOARD_GPIOB_BK4819_SCL
                   //
                   | BOARD_GPIOB_TX_BIAS_LDO | BOARD_GPIOB_TX_AMP_SEL |
                   BOARD_GPIOB_KEY_COL2 | BOARD_GPIOB_KEY_COL1 |
                   BOARD_GPIOB_KEY_COL0;
  ...
  // GPIO C
  init.gpio_pins =
      BOARD_GPIOC_BK1080_SEN | BOARD_GPIOC_BK1080_SCL | BOARD_GPIOC_LCD_CS;
  ...
  // GPIO A
  init.gpio_pins =
      BOARD_GPIOA_KEY_ROW3 | BOARD_GPIOA_KEY_ROW0 | BOARD_GPIOA_KEY_SIDE2;
