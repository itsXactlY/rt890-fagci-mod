...
void UI_DrawMain(bool bSkipStatus) {
  ...
  if (gSettings.DualDisplay == 0 &&
      (gRadioMode != RADIO_MODE_RX || gSettings.CurrentVfo == gCurrentVfo)) {
...
void UI_DrawRepeaterMode(void) {
  if (gSettings.RepeaterMode) {
    ...
    UI_DrawStatusIcon(109, gSettings.RepeaterMode == 1 ? ICON_TR : ICON_RR,
...
void UI_DrawBatteryBar() {
  ...
  uint8_t v = ConvertDomain(i, 0, 15, 0, 159);
  DISPLAY_DrawRectangle0(0, 95, v, 1, Color);
  DISPLAY_DrawRectangle0(v, 95, 160 - v, 1, gColorBackground);
}
