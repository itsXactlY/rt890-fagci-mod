...
uint16_t BK4819_GetRSSI(void) { return BK4819_ReadRegister(0x67) & 0x01FF; }

uint8_t BK4819_GetNoise(void) { return BK4819_ReadRegister(0x65) & 0x7F; }

uint8_t BK4819_GetGlitch(void) { return BK4819_ReadRegister(0x63) & 0xFF; }
...
void BK4819_Init(void) {
  ...
  BK4819_SetAFResponseCoefficients(false, false,
                                   gCalibration.RX_300Hz_Coefficient);
  BK4819_SetAFResponseCoefficients(true, true,
                                   gCalibration.TX_3000Hz_Coefficient);
  BK4819_SetAFResponseCoefficients(true, false,
                                   gCalibration.TX_300Hz_Coefficient);
  BK4819_EnableRX();
...
void BK4819_SetSquelchGlitch(bool bIsNarrow) {
  ...
  static const uint8_t gSquelchGlitchLevel[11] = {
      0x20, 0x20, 0x1E, 0x1C, 0x1A, 0x18, 0x16, 0x14, 0x12, 0x10, 0x0E,
  };
...
void BK4819_SetSquelchNoise(bool bIsNarrow) {
  ...
  static const uint8_t gSquelchNoiseLevel[11] = {
      0x0A, 0x0A, 0x09, 0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01,
  };
  ...
  if (bIsNarrow) {
    Value = ((gSquelchNoiseNarrow + 12 + Level) << 8) |
            (gSquelchNoiseNarrow + 6 + Level);
  } else {
    Value = ((gSquelchNoiseWide + 12 + Level) << 8) |
            (gSquelchNoiseWide - 6 + Level);
  }
...
void BK4819_SetSquelchRSSI(bool bIsNarrow) {
  ...
  static const uint8_t gSquelchRssiLevel[11] = {
      0x00, 0x00, 0x02, 0x04, 0x06, 0x0A, 0x10, 0x16, 0x1C, 0x22, 0x26,
  };
  ...
  if (bIsNarrow) {
    Value = ((gSquelchRSSINarrow - 8 + Level) << 8) |
            (gSquelchRSSINarrow - 14 + Level);
  } else {
    Value =
        ((gSquelchRSSIWide - 8 + Level) << 8) | (gSquelchRSSIWide - 14 + Level);
  }
...
void BK4819_ToggleAGCMode() {
  ...
  if (!(Value & 0x8000U) ||
      AGCIndex == 4) { // Bit 15 = 0 (AGC on) or 14:12 = 110 (index 4/min)
...
void BK4819_EnableRfTxDeviation(void) {
  ...
  Deviation = gMainVfo->bIsNarrow ? gFrequencyBandInfo.TxDeviationNarrow
                                  : gFrequencyBandInfo.TxDeviationWide;
...
void BK4819_StopFrequencyScan(void) { BK4819_WriteRegister(0x32, 0x0000); }
...
void BK4819_set_rf_frequency(const uint32_t frequency,
                             const bool trigger_update) {
  BK4819_WriteRegister(0x38, (frequency >> 0) & 0xFFFF);
  BK4819_WriteRegister(0x39, (frequency >> 16) & 0xFFFF);

  if (trigger_update) { // trigger a PLL/VCO update
